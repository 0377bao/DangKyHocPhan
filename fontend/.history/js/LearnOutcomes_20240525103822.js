    ...
    function updateResultsTable(data) {
        ...
        data.forEach((semester) => {
            ...
            semester.ketQua.forEach((result, index) => {
                var averageFrequent = averageScore(result.diemThuongXuyen)
                var averagePractice = averageScore(result.diemThucHanh)
                ...
                row.innerHTML = `
                    ...
                    <tr>${result.diemThuongXuyen.map(score => {
                        return `
                            <td>${formatScore(score)}</td>
                        `
                    })}
                    </tr>
                    <tr>${result.diemThucHanh.map(score=> {
                        return `
                            <td>${formatScore(score)}</td>
                        `
                    })}
